...
check_required_files() {
    ...
    if [ ! -f "$PLOT_TRACKER" ]; then
        echo "⚠️  Файл отслеживания сюжета не найден, создается..." >&2
...
check_foreshadowing() {
    ...
    if [ -f "$PLOT_TRACKER" ]; then
        ...
        if [ "$OVERDUE" -gt 0 ]; then
            echo ""
            echo "⚠️ Предупреждение: ${OVERDUE} завязок не обрабатывались более 30 глав."
        fi
    fi
}
...
generate_suggestions() {
    ...
    if [ "$CURRENT_CHAPTER" -lt 10 ]; then
        echo "• Первые 10 глав — ключевые, убедитесь, что есть достаточно крючков для привлечения читателя."
        ...
        echo "• Приближается конец тома, подготовьте кульминацию и задел на будущее."
    fi

    # Предложения на основе состояния завязок
    ...
    if [ "$ACTIVE_CONFLICTS" -eq 0 ] && [ "$CURRENT_CHAPTER" -gt 5 ]; then
        echo "• Текущих конфликтов нет, рассмотрите возможность введения новых точек противоречия."
    fi
}

# Основная функция
main() {
    echo "🔍 Начало проверки согласованности сюжета..."
    echo ""
