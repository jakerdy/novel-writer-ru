```bash
#!/usr/bin/env bash
# Текстовый аудит на естественность (офлайн): плотность связующих слов/пустых фраз, статистика длины предложений, плотность абстрактных слов
...
if [ -z "$FILE_PATH" ] || [ ! -f "$FILE_PATH" ]; then
  echo "Использование: scripts/bash/text-audit.sh <file>"
  exit 1
fi
...
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
print("📊 Отчет по проверке естественности текста (офлайн)")
...
print("")
print("Абстрактная перегрузка (примеры фрагментов, ≥2 абстрактных слов)")
if abstract_top:
  for idx, s in abstract_top:
    snippet = s[:80] + ("…" if len(s)>80 else "")
    print(f"  - Предложение {idx+1}: {snippet}")
else:
  print("  Значительных фрагментов с абстрактной перегрузкой не обнаружено")

print("")
print("Рекомендации")
print("  - Заменяйте пустые фразы и абстрактные существительные конкретными действиями/предметами/запахами")
print("  - Разбавляйте длинные предложения; объединяйте слишком короткие для создания ритма")
print("  - Проверьте, можно ли удалить связующие слова или заменить их более естественными переходами")
print("  - Перед написанием составьте список из 3 бытовых деталей в качестве опоры")
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
PY
```