...
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
print("📊 Отчёт самопроверки текста на «человечность»")
...
print(f"  Всего: {total_conn}  | Соотношение: {ratio(total_conn):.2f}")
for k,v in sorted(connectors.items(), key=lambda x: -x[1])[:10]:
  if v>0: print(f"  - {k}: {v}")

print("")
print("Подсчёт пустых фраз/клише")
total_emp = sum(empties.values())
print(f"  Всего: {total_emp}  | Соотношение: {ratio(total_emp):.2f}")
...
print("")
print("Перегрузка абстракциями (примеры фрагментов, ≥2 абстрактных слов)")
if abstract_top:
  for idx, s in abstract_top:
    snippet = s[:80] + ("…" if len(s)>80 else "")
    print(f"  - Предложение №{idx+1}: {snippet}")
else:
  print("  Существенных фрагментов с перегрузкой абстракциями не обнаружено")

print("")
print("Рекомендации")
print("  - Заменяйте пустые фразы и абстрактные существительные конкретными действиями/предметами/запахами")
print("  - Разбивайте слишком длинные цепочки предложений; объединяйте избыточно короткие, чтобы создать динамику")
