...
check_timeline_consistency() {
    ...
    if [ -f "$TIMELINE" ]; then
        # Проверка, увеличиваются ли события по главам
        TIMELINE_ISSUES=$(jq '
            .events |
            sort_by(.chapter) |
            . as $sorted |
...
# Проверка логичности состояния персонажей
check_character_consistency() {
    echo "👥 Проверка логичности состояния персонажей"
    echo "─────────────────────"
...
# Проверка плана по отработке сюжетных заделов
check_foreshadowing_plan() {
    echo "🎯 Проверка управления сюжетными заделами"
    echo "──────────────"

    if [ -f "$PLOT_TRACKER" ]; then
        # Статистика статусов сюжетных заделов
        ...
        if [ -f "$PROGRESS" ]; then
            ...
            # Проверка просроченных сюжетных заделов
            ...
            check "Своевременная отработка сюжетных заделов" \
                  "[ '$OVERDUE' = '0' ]" \
                  "просрочено ${OVERDUE} сюжетных заделов (более 50 глав)"

        fi

        echo "  📊 Статистика сюжетных заделов: всего ${TOTAL_FORESHADOW}, активно ${ACTIVE_FORESHADOW}"

        # Предупреждение о слишком большом количестве активных заделов
        if [ "$ACTIVE_FORESHADOW" -gt 10 ]; then
            warn "Слишком много активных сюжетных заделов (${ACTIVE_FORESHADOW} шт.), это может сбить с толку читателей"
        fi
    else
