...
if [ ! -f "memory/writing-constitution.md" ] && [ ! -f ".specify/memory/writing-constitution.md" ]; then
    missing+=("Конституция написания")
fi

if [ ! -f "$SPEC_FILE" ]; then
    missing+=("Файл спецификации")
fi
...
if [ ${#missing[@]} -gt 0 ]; then
    ...
    if [ ! -f "memory/writing-constitution.md" ] && [ ! -f ".specify/memory/writing-constitution.md" ]; then
        echo "  1. /constitution - Создание конституции написания"
    fi
    if [ ! -f "$SPEC_FILE" ]; then
...
if [ -f "$PLAN_FILE" ]; then
    echo ""
    echo "📋 Файл плана уже существует, существующий план будет обновлен"
