...
**验证方法**：
```markdown
Используйте скрипт подсчета слов, предоставленный в проекте, для проверки количества слов в главе:
```bash
source scripts/bash/common.sh
count_chinese_words "stories/*/content/第X章.md"
```

⚠️ **Внимание**: Не используйте `wc -w` для подсчета китайских слов, он крайне неточен для китайского языка!
...
- Требование к количеству слов: 2000-4000 слов
- Статус количества слов: ✅ Соответствует требованиям / ⚠️ Недостаточно слов / ⚠️ Превышено количество слов
- Статус задачи: Обновлено
```

### 9. Рекомендации по дальнейшим действиям
- Продолжить следующую задачу по написанию
- Запускайте `/analyze` каждые 5 глав для проверки качества
- Своевременно корректируйте план при обнаружении проблем
...
/plan → Разрабатывает техническое решение
     ↓
/tasks → Декомпозирует задачи для выполнения
     ↓
/write → 【Текущее】Выполнение написания
     ↓
/analyze → Проверяет соответствие качества
```

Помните: написание — это уровень выполнения, и оно должно строго следовать спецификациям и плану верхнего уровня.
```