...
Этот проект предоставляет следующие команды с косой чертой:

### Основные команды
- `/method` - Помощник по интеллектуальным методам письма
...
- `/relations` - Управление отношениями персонажей
...
- `/world:check` - Проверка согласованности мира
...
- Выполнение команд оболочки (ограниченная область)
...
Gemini CLI может иногда выводить отдельные китайские символы с ошибками (отображаются как  или другие нечитаемые символы), это известная проблема кодировки Gemini.
...
#### Решение при появлении нечитаемых символов
1. **Повторная генерация**: повторно выполните ту же команду, обычно второй раз получается нормально
2. **Ручное исправление**: напрямую отредактируйте сгенерированный файл, заменив нечитаемые символы правильными китайскими
3. **Обработка по частям**: если какая-либо секция отображается некорректно, перегенерируйте только эту секцию

#### Распространенные шаблоны нечитаемых символов
- `` → Обычно это часто используемые иероглифы, такие как "的", "了"
- `\u4e2d\u6587` → Unicode-экранирование, необходимо преобразовать обратно в китайский
- Часть знаков препинания отображается некорректно → вручную замените китайскими знаками препинания

#### Временное решение
Если нечитаемые символы появляются часто, вы можете:
1. Генерировать более короткие абзацы (уменьшить объем вывода за один раз)
2. Использовать стандартный режим вместо режима секций
3. Проверить и исправить сразу после генерации

Примечание: Google работает над исправлением этой проблемы, и последующие версии должны улучшить ситуацию.
