...
# Скрипт проверки статуса написания
...
# Проверка, находится ли скрипт в режиме checklist
...
- [$([ "$has_constitution" = true ] && echo "x" || echo " ")] CHK001 constitution.md присутствует
...
EOF
    ...
    if [ "$has_tasks" = true ]; then
        ...
        echo "- [ ] CHK005 Есть задачи в работе (tasks.md отсутствует)"
        echo "- [ ] CHK006 Количество ожидающих задач (tasks.md отсутствует)"
...
EOF
    ...
    if [ $chapter_count -gt 0 ]; then
        echo "- [$([ $bad_chapters -eq 0 ] && echo "x" || echo "!")] CHK009 Объем соответствует стандарту ($([ $bad_chapters -eq 0 ] && echo "Все соответствуют" || echo "$bad_chapters глав не соответствуют"))"
    else
...
EOF
    ...
    if [ $pending -gt 0 ] || [ $in_progress -gt 0 ]; then
        if [ $in_progress -gt 0 ]; then
            echo "- [ ] Продолжить работу над текущими задачами ($in_progress шт.)"
        else
    ...
    if [ "$has_actions" = false ]; then
        echo "*Статус написания в порядке, специальных действий не требуется*"
    fi
...
# Основной процесс
main() {
    # В режиме checklist выводим результат и завершаем работу
    if [ "$CHECKLIST_MODE" = true ]; then
        output_checklist
        exit 0
    fi

    # Оригинальный режим подробного вывода
    echo "Проверка статуса написания"
    echo "============"
