...
if [ "$story_exists" = false ] || [ "$outline_exists" = false ]; then
    ...
    echo "   Отсутствуют: ${story_exists:+}${story_exists:-specification.md} ${outline_exists:+}${outline_exists:-outline.md}"
    exit 1
fi

# Создание директории отслеживания
    ...
    cat > spec/tracking/plot-tracker.json <<EOF
{
  ...
  "notes": {
    "plotHoles": [],
    "inconsistencies": [],
    "reminders": ["Пожалуйста, обновляйте данные отслеживания в соответствии с фактическим содержанием истории"]
  }
}
EOF
fi
...
echo ""
echo "💡 Следующие шаги:"
echo "   1. Начните писать с помощью /write (данные отслеживания будут обновляться автоматически)"
