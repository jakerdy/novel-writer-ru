...
# Скрипт проверки состояния письма
...
check_methodology_docs() {
    ...
    [ ! -f "memory/constitution.md" ] && missing+=("Конституция")
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        ...
        echo ""
        echo "Рекомендуется выполнить предварительные шаги в соответствии с семиэтапной методологией:"
        echo "1. /constitution - Создать конституцию творчества"
        echo "2. /specify - Определить спецификацию истории"
        echo "3. /clarify - Уточнить ключевые решения"
        echo "4. /plan - Составить план творчества"
        echo "5. /tasks - Сгенерировать список задач"
        return 1
    fi
...
check_pending_tasks() {
    ...
    # Отобразить следующую задачу для написания
    echo ""
...
check_completed_content() {
    ...
    if [ -d "$content_dir" ]; then
        ...
        if [ $chapter_count -gt 0 ]; then
            echo ""
            echo "Завершено глав: $chapter_count"
        ...
        echo ""
        echo "Писательство еще не начато"
    fi
}
...
output_checklist() {
    ...
    # Вывод чек-листа
    cat <<EOF
# Чек-лист проверки состояния письма
...
- [$([ "$has_constitution" = true ] && echo "x" || echo " ")] CHK001 constitution.md существует
...
EOF
    ...
    if [ $chapter_count -gt 0 ]; then
        echo "- [$([ $bad_chapters -eq 0 ] && echo "x" || echo "!")] CHK009 Объем соответствует стандарту ($([ $bad_chapters -eq 0 ] && echo "Все соответствуют" || echo "$bad_chapters шт. не соответствуют"))"
    else
        echo "- [ ] CHK009 Объем соответствует стандарту (писательство еще не начато)"
    fi
...
EOF
    ...
    if [ "$has_constitution" = false ] || [ "$has_specification" = false ] || [ "$has_plan" = false ] || [ "$has_tasks" = false ]; then
        echo "- [ ] Завершить документы методологии (выполнить соответствующие команды: /constitution, /specify, /plan, /tasks)"
        has_actions=true
    fi
    ...
    if [ $bad_chapters -gt 0 ]; then
        echo "- [ ] Исправить главы, не соответствующие требованиям по объему ($bad_chapters шт.)"
        has_actions=true
    fi
    ...
    if [ "$has_actions" = false ]; then
        echo "*Состояние письма хорошее, никаких особых действий не требуется*"
    fi
...
# Основной поток
main() {
    # Режим Checklist: прямой вывод и выход
    ...
    echo "Проверка состояния письма"
    echo "============"
