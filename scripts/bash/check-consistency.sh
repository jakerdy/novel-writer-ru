...
check_chapter_consistency() {
    ...
    if [ -f "$PROGRESS" ] && [ -f "$PLOT_TRACKER" ]; then
        ...
        if [ -f "$CHARACTER_STATE" ]; then
            ...
            check "Синхронизация глав в состоянии персонажа" \
...
# Проверка логичности состояния персонажа
check_character_consistency() {
    echo "👥 Проверка логичности состояния персонажа"
    echo "─────────────────────"
    ...
    if [ -f "$CHARACTER_STATE" ] && [ -f "$RELATIONSHIPS" ]; then
        ...
        if [ -n "$PROTAG_NAME" ]; then
            ...
            check "Запись отношений главного героя" \
                  "[ '$HAS_RELATIONS' = 'true' ]" \
                  "главный герой '$PROTAG_NAME' отсутствует в relationships.json"
        fi
...
# Проверка плана по отслеживанию зацепок
check_foreshadowing_plan() {
    ...
    if [ -f "$PLOT_TRACKER" ]; then
        ...
        if [ -f "$PROGRESS" ]; then
            ...
            # Проверка просроченных зацепок, которые не были разрешены
            ...
            check "Своевременное разрешение зацепок" \
                  "[ '$OVERDUE' = '0' ]" \
                  "${OVERDUE} зацепки не разрешены более 50 глав"
        fi

        echo "  📊 Статистика зацепок: всего ${TOTAL_FORESHADOW}, активно ${ACTIVE_FORESHADOW}"

        # Предупреждение о слишком большом количестве активных зацепок
        if [ "$ACTIVE_FORESHADOW" -gt 10 ]; then
            warn "Слишком много активных зацепок (${ACTIVE_FORESHADOW} шт.), что может сбить с толку читателей"
        fi
    else
...
EOF
    ...
    if [ -n "$char_chapter" ]; then
        ...
        echo "- [ ] CHK007 Проверка номера главы в character-state.json (файл отсутствует или данные не найдены)"
    fi
...
EOF
    if [ -f "$CHARACTER_STATE" ] && [ -f "$RELATIONSHIPS" ]; then
        ...
        if [ -n "$protag_name" ]; then
            echo "- [x] CHK010 Основная информация о протагонисте полная（$protag_name）"
            ...
            echo "- [ ] CHK010 Основная информация о протагонисте полная（отсутствуют данные）"
            echo "- [ ] CHK011 Запись отношений протагониста（отсутствуют данные）"
        fi
        ...
        echo "- [ ] CHK010 Основная информация о протагонисте полная（файл не существует）"
        echo "- [ ] CHK011 Запись отношений протагониста（файл не существует）"
...
EOF
    ...
    if [ "$total_foreshadow" -gt 0 ]; then
        echo "- [x] CHK013 Записи намёков существуют（всего $total_foreshadow, активно $active_foreshadow）"

        if [ "$overdue_foreshadow" -eq 0 ]; then
            echo "- [x] CHK014 Намёки своевременно закрываются（нет просроченных）"
        else
            echo "- [!] CHK014 Намёки своевременно закрываются（$overdue_foreshadow просрочено более чем на 50 глав）"
        fi

        if [ "$active_foreshadow" -le 10 ]; then
            echo "- [x] CHK015 Количество активных намёков в норме（$active_foreshadow ≤ 10）"
        else
            echo "- [!] CHK015 Слишком много активных намёков（$active_foreshadow > 10, может сбить с толку читателей）"
        fi
    else
        echo "- [ ] CHK013 Записи намёков существуют（записи не найдены）"
        echo "- [ ] CHK014 Намёки своевременно закрываются（нет данных）"
        echo "- [ ] CHK015 Количество активных намёков в норме（нет данных）"
    fi
...
EOF
    ...
    if [ "$WARNINGS" -gt 0 ]; then
        echo "- [ ] Обратить внимание на предупреждения, рассмотреть возможность улучшений"
    fi
    if [ "$ERRORS" -eq 0 ] && [ "$WARNINGS" -eq 0 ]; then
...
main() {
    ...
    # Возвращаем соответствующий код выхода в зависимости от результата
    if [ "$ERRORS" -gt 0 ]; then
        exit 1
