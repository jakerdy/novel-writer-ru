...
- ⭐ 背景：偶尔提及，保持存在感
```
- ❌ Не появилось: зацепка ещё не активирована

### 5.3 Планирование точек пересечения зацепок

Предварительно спланируйте точки пересечения между зацепками, чтобы избежать произвольных действий ИИ:

| ID точки пересечения | Глава | Задействованные зацепки | Содержание пересечения | Ожидаемый эффект |
|---------|------|---------|---------|---------|
| X-001 | [Номер главы] | PL-XX+PL-YY | [Как две/несколько зацепок пересекаются, что происходит] | [Влияние на историю и персонажей] |
...
- Задействованные зацепки: перечислите все ID зацепок, пересекающихся в этой точке
- Содержание пересечения: конкретный сюжет или событие
- Ожидаемый эффект: эмоциональный конфликт, поворот сюжета, рост персонажа и т. д.

### 5.4 Таблица управления предзнаменованиями

Управляйте установкой и раскрытием всех предзнаменований, чтобы ничего не упустить:

| ID предзнаменования | Глава установки | Задействованные зацепки | Содержание предзнаменования | Глава раскрытия | Способ раскрытия |
|-------|---------|---------|---------|---------|---------|
| F-001 | [Номер главы] | PL-XX | [Какое предзнаменование заложено, конкретное содержание] | [Номер главы] | [Как раскрывается, через какое событие] |
| F-002 | [Номер главы] | PL-XX+PL-YY | [Содержание предзнаменования, охватывающего несколько зацепок] | [Номер главы] | [Способ раскрытия] |

**Пояснения**:
- ID предзнаменования: унифицированный формат: F-XXX (первая буква от Foreshadowing)
- Задействованные зацепки: зацепки, связанные с предзнаменованием, возможно, охватывающие несколько зацепок
- Номера глав установки и раскрытия должны быть четко указаны, чтобы избежать забывания

### 5.5 Матрица принятия решений по изменению зацепок

При необходимости изменения какой-либо зацепки обязательно оцените влияние в соответствии с этим процессом:
...
3. Проверьте раздел 5.4: Какие предзнаменования задействует эта зацепка? Нужно ли корректировать установку/раскрытие предзнаменований?
4. Проверьте creative-plan.md: Какие разделы глав нужно синхронно изменить?
5. Проверьте tasks.md: Какие задачи по написанию нужно перепланировать?
6. Проверьте plot-tracker.json: Каков текущий прогресс, как его скорректировать в дальнейшем?

**Пример**: Предположим, мы изменяем PL-03 (любовная линия), перенося появление персонажа на 50-ю главу (изначально планировалось на 100-ю):
- ✅ Точка пересечения X-004 должна быть перенесена или отменена
- ✅ Активные зацепки в разделах глав второго тома должны быть скорректированы
- ✅ Поле "Задействованные зацепки" в соответствующих задачах должно быть обновлено
- ✅ Может повлиять на время раскрытия предзнаменования F-002
...
- Зацепки, находящиеся в длительной спячке (>20 глав), должны иметь разумное обоснование

**Критерии проверки**:
- [ ] Все зацепки имеют четкие конфликты и решения
- [ ] Распределение темпа зацепок равномерное, без чрезмерной концентрации или пробелов
- [ ] Количество точек пересечения умеренное (рекомендуется 2-3 на 50 глав)
- [ ] Все предзнаменования имеют соответствующие планы раскрытия
- [ ] Матрица принятия решений по изменениям выполнима

## Шесть. Ограничительные условия
...
- Временные ограничения: [срок выполнения]
...
- Требования к формату: [например, длина глав]
- Требования к обновлению: [например, фиксированное время]

## Семь. Оценка рисков

### Творческие риски
- Сложность написания: [требуется уточнение: в чем заключаются трудности]
- Истощение вдохновения: [как реагировать]
...
- Своевременность: [не устареет ли тема]

## Восемь. Ключевые точки принятия решений [требуется уточнение]

Следующие ключевые решения должны быть определены на этапе `/clarify`:
1. [Решение 1: например, характер главного героя — горячий или спокойный]
2. [Решение 2: например, финал — открытый или счастливый]
...
## Девять. Контрольный список проверки

- [ ] Краткое изложение истории четкое и ясное
- [ ] Определение целевой аудитории точное
- [ ] Критерии успеха измеримы
- [ ] Основные потребности перечислены
- [ ] Спецификации управления зацепками определены
- [ ] Ограничительные условия выявлены
- [ ] Риски оценены
...
### Референтные произведения
- [Произведение 1]: ссылка на [особенность]
- [Произведение 2]: ссылка на [особенность]

### Дополнительные пояснения
[Другая информация, требующая пояснений]
```
...
- **Уровень 1**: Вывод однопредложенческой спецификации + идентификация ключевых элементов + руководство по расширению
- **Уровень 2**: Вывод многопредложенческой спецификации + ключевые элементы + позиционирование типа + руководство по расширению
- **Уровень 3**: Вывод одностраничной спецификации + трехактная структура + ключевые сцены + руководство по расширению
- **Уровень 4**: Вывод полной девятиглавной спецификации + метка `[требуется уточнение]`

**Подсказка после вывода**:

Для Уровня 1-3 сообщите пользователю:
```
✅ Спецификация Уровня X завершена!

**Текущий прогресс**: Одно предложение → Несколько предложений → Одна страница → Полная спецификация
            [=====---] (Вы здесь)

**Следующие варианты**:
1. Использовать текущую спецификацию для начала работы (подходит для быстрого прототипирования)
2. Расширить до следующего уровня (более детальное планирование)
3. Запустить `/clarify` для уточнения текущей спецификации

**Как расширить**:
- Предоставить более подробное описание истории
- Снова запустить `/specify [подробное описание]`
- Система автоматически обновится до Уровня X+1
```
...
**Следующий шаг**:
1. Запустите `/clarify` для уточнения всех помеченных как `[требуется уточнение]` точек принятия решений
2. Или запустите `/plan` для начала разработки плана работы
```

### 5. Отметка точек, требующих уточнения (только Уровень 4)
...
- Они будут обработаны на шаге `/clarify`

**Уровни 1-3 не требуют отметки точек уточнения**, поскольку они сами по себе являются процессом постепенного уточнения

### 6. Управление версиями

**Правила инкрементного версионирования**:

- **Уровень 1**: 1.0.0-L1 (одно предложение)
- **Уровень 2**: 1.1.0-L2 (несколько предложений)
...
- Уровень 3 → Уровень 4: сброс до 1.0.0 (вход в цикл официальной спецификации)
- Последующие изменения после Уровня 4: по исходным правилам

### 7. Вывод и сохранение

- Сохраните спецификацию в `stories/[story-name]/specification.md`
- Выведите сообщение об успешном создании
- **В зависимости от уровня, выведите подсказку о следующем шаге**:
  - Уровень 1-3: подсказка о том, как расширить до следующего уровня
  - Уровень 4: подсказка о запуске `/clarify` для уточнения ключевых решений

## Примечания

### Ценность инкрементной спецификации

**Зачем нужна инкрементная спецификация?**
- ✅ **Снижение порога входа**: Начните с одного предложения, нет необходимости сразу создавать полную спецификацию
- ✅ **Быстрая проверка**: Проверьте осуществимость идеи, прежде чем вкладывать много усилий
- ✅ **Естественная эволюция**: Естественным образом расширяйте детали спецификации по мере углубления размышлений
- ✅ **Избегание избыточного планирования**: Определите глубину спецификации в зависимости от фактических потребностей

**Когда использовать каждый уровень?**
- **Уровень 1**: Этап идеи, быстрое фиксирование концепции
- **Уровень 2**: Этап проверки, подтверждение того, стоит ли писать историю
- **Уровень 3**: Этап прототипирования, быстрое начало работы (подходит для коротких и средних произведений)
- **Уровень 4**: Официальный этап, для романов или произведений, требующих детального планирования
...
- Оставляйте пространство для уточнений (Уровень 4)
...
- **Прирост**: Добавляйте только необходимые детали каждый раз
- **Обратимость**: Можно вернуться к более низкому уровню для изменения ключевых элементов
- **Возможность пропуска**: Можно напрямую перейти с Уровня 1 на Уровень 4 (если все уже продумано)

### Связь с последующими шагами
- **Уровень 1-3**: Можно сразу приступать к написанию (подходит для исследовательского творчества)
- **Уровень 4**: Запустите `/clarify` для обработки всех пометок `[требуется уточнение]`
- **После уточнения**: Запустите `/plan` для составления плана работы
- **При выполнении**: Запустите `/analyze` для проверки соответствия реализации спецификации
