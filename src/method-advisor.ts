```typescript
/**
 * Метод интеллектуальной системы рекомендаций
 * Рекомендует наиболее подходящий метод письма на основе характеристик произведения
 */
...
export class MethodAdvisor {
  private methodProfiles = {
    ...
    'seven-point': {
      ...
      audiences: ['Массовая', 'Коммерческие читатели'],
    ...
    'pixar-formula': {
      genres: ['Детский', 'Короткий', 'Душевный', 'Притча'],
      lengthRange: { min: 5000, max: 50000 },
      audiences: ['Детский', 'Семейный', 'Все возрасты'],
  ...
  /**
   * Рекомендует наиболее подходящий метод письма
   */
  ...
  private calculateScore(features: StoryFeatures, profile: any): { total: number; reasons: string[] } {
    ...
    // Соответствие основного акцента (вес: 10)
    if (profile.focus.includes(features.focus)) {
      score += 10;
      reasons.push(`Сильная сторона в описании ${features.focus}`);
    }
    ...
    if (profile.complexity.includes(features.complexity)) {
      score += 5;
      reasons.push('Уровень сложности подходит');
    }
  ...
  private analyzeMatch(features: StoryFeatures, profile: any, method: string): { pros: string[]; cons: string[] } {
    ...
    if (profile.genres.includes(features.genre)) {
      pros.push('Высокое соответствие жанру');
    }
    if (profile.experience.includes(features.experience)) {
      pros.push('Уровень сложности подходящий');
    }
    if (features.length >= profile.lengthRange.min && features.length <= profile.lengthRange.max) {
      pros.push('Объем подходящий');
    }
  ...
  /**
   * Получение подробной рекомендации
   */
  getDetailedRecommendation(features: StoryFeatures): string {
    ...
    let recommendation = `## 📊 Отчет о рекомендуемом методе письма\n\n`;
    recommendation += `### Анализ характеристик произведения\n`;
    recommendation += `- Жанр: ${features.genre}\n`;
    recommendation += `- Объем: ${(features.length / 10000).toFixed(1)}万字\n`;
    recommendation += `- Аудитория: ${features.audience}\n`;
    recommendation += `- Опыт: ${features.experience}\n`;
    recommendation += `- Акцент: ${features.focus}\n`;
    ...
    recommendation += `**Степень соответствия: ${top.score}%**\n\n`;
    ...
    if (second && second.score >= 70) {
      ...
      recommendation += `**Степень соответствия: ${second.score}%**\n\n`;
      recommendation += `**Причины рекомендации:**\n`;
  ...
  /**
   * Получение названия метода на китайском
   */
  ...
  private getSpecificTips(method: string, features: StoryFeatures): string {
    const tips: Record<string, string> = {
      'three-act': `
- Первая часть должна занимать около 25% произведения, быстро создавая конфликт.
- Во второй части избегайте затянутости, можно ввести несколько небольших кульминаций.
- Третья часть должна быть сжатой и сильной, не завершайте ее слишком поспешно.`,
      'hero-journey': `
...
- Каждый шаг должен способствовать внутренним изменениям персонажа.
- Можно вкладывать малые циклы в большой для увеличения глубины.`,
      'seven-point': `
- Убедитесь, что каждый пункт действительно продвигает сюжет.
- Центральная точка должна быть настоящим поворотом, меняющим правила игры.
- Не пропускайте заключительные пункты, они важны для поддержания напряжения.`,
      'pixar-formula': `
- Сохраняйте лаконичность, не вдавайтесь в излишние описания.
- Подчеркните четкую связь причинно-следственных связей.
- Концовка должна быть удовлетворительной, но может оставлять пищу для размышлений.`
    };
...
export function recommendHybrid(features: StoryFeatures): string {
  ...
  if (features.length > 100000 && (features.genre === 'Фэнтези' || features.genre === 'Приключения')) {
    recommendations.push('Основная линия использует "Путешествие героя"');
  } else if (features.genre === 'Детектив') {
    recommendations.push('Основная линия использует "Семь точек"');
  } else {
    recommendations.push('Основная линия использует "Три акта"');
  }

  // Побочные линии
  if (features.focus === 'Персонажи') {
    recommendations.push('Побочные линии персонажей используют "Круг истории"');
  }

  // Структура глав
  if (features.pace === 'Быстрый') {
    recommendations.push('Отдельные главы могут быть организованы по "Формуле Пиксара"');
  }
