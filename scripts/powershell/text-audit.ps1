...
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
print("📊 Отчёт автоматической проверки текста")
print(f"Файл: {os.path.basename(path)}  Кол-во символов: {total_chars}")
print("")
print("Плотность связующих фраз (на тысячу символов)")
...
print(f"  Кол-во предложений: {len(lens)}  | Средняя: {avg:.1f}  | Стандартное отклонение: {std:.1f}")
print(f"  Макс. последовательность коротких: {mx_run_short} (порог {cfg['sentence_length']['max_run_short']})")
print(f"  Макс. последовательность длинных: {mx_run_long} (порог {cfg['sentence_length']['max_run_long']})")
print("")
print("Абстрактная перегрузка (примеры, ≥2 абстрактных слов)")
if abstract_top:
...
print("  - Проверьте, можно ли удалить связующие слова или заменить их естественным переходом")
print("  - Перед написанием составьте 3 бытовых детали в качестве опорных точек")
PY